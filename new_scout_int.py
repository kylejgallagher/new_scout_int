...
file = "scout from 1-2024 to 10-28-2025.csv"
...
# ===== Keep only first message per resume_contact_id/resume_id =====
filtered = filtered.sort_values(by="date_created")
filtered = filtered.groupby(["resume_contact_id", "resume_id"]).first().reset_index()

# ===== messages_until_keyword per resume_contact_id/resume_id =====
filtered["messages_until_keyword"] = filtered.groupby(["resume_contact_id", "resume_id"])["normalized_response"] \
    .cumcount() + 1
...
# ===== Final columns =====
final = filtered[[
    "resume_contact_id", "date_created", "employer_id", "division_id",
    "job_id", "resume_id", "job_seeker_id", "response", "Direct", "type"
]]
